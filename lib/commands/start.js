...
async function start(siteDirParam = '.', cliOptions = {}) {
    ...
    // See https://github.com/facebook/ianaio/issues/4542
    ...
    logger_1.default.success `IANA SYSTEMS website is running at: url=${openUrl}`;
    ...
    const reload = lodash_1.default.debounce(() => {
        loadSite()
            ...
            if (newOpenUrl !== openUrl) {
                logger_1.default.success `IANA SYSTEMS website is running at: url=${newOpenUrl}`;
            }
        })
            .catch((err) => {
    ...
    let config = (0, webpack_merge_1.default)(await (0, client_1.default)(props, cliOptions.minify), {
        watchOptions: {
            ignored: /node_modules\/(?!@ianaio)/,
            poll: cliOptions.poll,
        },
        infrastructureLogging: {
            // Reduce log verbosity, see https://github.com/facebook/ianaio/pull/5420#issuecomment-906613105
            level: 'warn',
        },
        plugins: [
