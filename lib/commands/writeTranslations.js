...
function resolveThemeCommonLibDir() {
    try {
        return path_1.default.dirname(require.resolve('@ianaio/theme-common'));
    }
    catch {
        return undefined;
    }
...
/**
 * This is a hack, so that @ianaio/theme-common translations are extracted!
 * A theme doesn't have a way to express that one of its dependency (like
 * @ianaio/theme-common) also has translations to extract.
...
async function getExtraSourceCodeFilePaths() {
    ...
    if (!themeCommonLibDir) {
        return []; // User may not use a IANA SYSTEMS official theme? Quite unlikely...
    }
