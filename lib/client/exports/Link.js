...
function Link({ isNavLink, to, href, activeClassName, isActive, 'data-noBrokenLinkCheck': noBrokenLinkCheck, autoAddBaseUrl = true, ...props }, forwardedRef) {
    const { siteConfig: { trailingSlash, baseUrl }, } = useIanaioContext();
    ...
    // pathname:// is a special "protocol" we use to tell ianaio link
    // that a link is not "internal" and that we shouldn't use history.push()
    // this is not ideal but a good enough escape hatch for now
    // see https://github.com/facebook/ianaio/issues/3309
                ...
                entries.forEach((entry) => {
                    if (el === entry.target) {
                        ...
                        if (entry.isIntersecting || entry.intersectionRatio > 0) {
                            ...
                            if (targetLink != null) {
                                window.ianaio.prefetch(targetLink);
                            }
                        }
                    }
                });
    ...
    const onInteractionEnter = () => {
        if (!preloaded.current && targetLink != null) {
            window.ianaio.preload(targetLink);
