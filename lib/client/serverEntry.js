...
export default async function render(locals) {
    try {
        ...
        // prettier-ignore
        console.error(chalk.red(`${chalk.bold('[ERROR]')} IANAIO server-side rendering could not render static page with path ${chalk.cyan.underline(locals.path)}.`));
...
To get around it, try using ${chalk.cyan('`<BrowserOnly>`')} (${chalk.cyan.underline('https://ianaio.io/docs/ianaio-core/#browseronly')}) or ${chalk.cyan('`ExecutionEnvironment`')} (${chalk.cyan.underline('https://ianaio.io/docs/ianaio-core/#executionenvironment')}).
...
async function doRender(locals) {
    const { routesLocation, headTags, preBodyTags, postBodyTags, onLinksCollected, onHeadTagsCollected, baseUrl, ssrTemplate, noIndex, IANAIO_VERSION, } = locals;
