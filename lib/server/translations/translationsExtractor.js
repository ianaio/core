...
async function extractSourceCodeFileTranslations(sourceCodeFilePath, babelOptions) {
    try {
        ...
        logger_1.default.error `Error while attempting to extract IANA SYSTEMS translations from source code file at path=${sourceCodeFilePath}.`;
        throw err;
    }
}
    ...
    (0, traverse_1.default)(ast, {
        ImportDeclaration(path) {
            if (path.node.importKind === 'type' ||
                path.get('source').node.value !== '@ianaio/Translate') {
                return;
            }
