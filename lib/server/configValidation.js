...
const utils_validation_1 = require("@ianaio/utils-validation");
...
{
  plugins: [
    ["@ianaio/plugin-content-docs",options],
    "./myPlugin",
    ...
    [function myPlugin() { },options]
            ...
            error.message = ` => Bad IANA SYSTEMS ${theme ? 'theme' : 'plugin'} value ${error.path.reduce((acc, cur) => typeof cur === 'string' ? `${acc}.${cur}` : `${acc}[${cur}]`)}.
${validConfigExample}
`;
        });
        return errors;
...
const SiteUrlSchema = utils_validation_1.Joi.string()
    ...
    try {
        ...
        if (pathname !== '/') {
            return helpers.error('ianaio.subPathError', { pathname });
        }
    }
    catch {
    ...
    'ianaio.subPathError': 'The url is not supposed to contain a sub-path like "{#pathname}". Please use the baseUrl field for sub-paths.',
});
// TODO move to @ianaio/utils-validation
exports.ConfigSchema = utils_validation_1.Joi.object({
    ...
    scripts: utils_validation_1.Joi.array()
        ...
        // See https://github.com/facebook/ianaio/issues/3378
        .unknown())
        .messages({
    ...
    'ianaio.configValidationWarning': 'IANA SYSTEMS config validation warning. Field {#label}: {#warningMessage}',
});
// TODO move to @ianaio/utils-validation
